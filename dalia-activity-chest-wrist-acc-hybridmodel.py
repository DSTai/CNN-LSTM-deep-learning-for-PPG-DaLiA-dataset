...
def read_data(cwd, filepath):
    ...
    for f in all_filenames:
        ...
        for l in range(0, int(len(list_wrist_ACC)/RESAMPLE_WRIST)):
            #ECG.append(list_chest_ECG[i][0])
            #RESP.append(list_chest_RESP[i*RESAMPLE][0])
            WRIST_ACC_X.append(list_wrist_ACC[i * RESAMPLE_WRIST][0])
            WRIST_ACC_Y.append(list_wrist_ACC[i * RESAMPLE_WRIST][1])
            WRIST_ACC_Z.append(list_wrist_ACC[i * RESAMPLE_WRIST][2])
            #EDA.append(list_chest_EDA[i][0])
            #EMG.append(list_chest_EMG[i][0])
            #EDA.append(list_wrist_EDA[i][0])
            #BVP.append(list_wrist_BVP[i][0])
...
def create_segments_and_labels(df, time_steps, step):
    ...
    for i in range(0, len(df) - time_steps, step):
        ...
        chest_acc_x = df['CHEST-ACC-X'].values[i: i + time_steps]
        chest_acc_y = df['CHEST-ACC-Y'].values[i: i + time_steps]
        chest_acc_z = df['CHEST-ACC-Z'].values[i: i + time_steps]
        wrist_acc_x = df['WRIST-ACC-X'].values[i: i + time_steps]
        wrist_acc_y = df['WRIST-ACC-Y'].values[i: i + time_steps]
        wrist_acc_z = df['WRIST-ACC-Z'].values[i: i + time_steps]
...
y_train = to_categorical(y_train - 1, num_classes=len(LABELS))
...
y_test = to_categorical(y_test- 1, num_classes=len(LABELS))
...
model_m.add(layers.Conv1D(filters=64, kernel_size=3, activation='relu', input_shape=input_shape))
model_m.add(layers.Conv1D(filters=64, kernel_size=3, activation='relu'))
model_m.add(layers.MaxPooling1D(pool_size=2))
model_m.add(layers.Dropout(0.5))

# Add an LSTM layer
model_m.add(layers.LSTM(100, return_sequences=True))
model_m.add(layers.LSTM(100))
model_m.add(layers.Dropout(0.5))
...
WEIGHTS_PATH = "D:/Documents/Downloads/Human_Activity_Recognition/weights/dalia-activity-chest-wrist-acc-hybrid/"
...
#write_list_to_file(act_list_actual_train,'semantic_concept_generation/activity_actual_train.csv')
    ...
    report_file_path = os.path.join(report_dir,f"{report_dir}/HYBRID_Chest_wrist_batch_size{BATCH_SIZE}.txt")
